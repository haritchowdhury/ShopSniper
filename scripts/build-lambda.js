...
export const LAMBDA_HANDLERS = Object.freeze([
  "discovery-worker",
  "domain-aggregator",
  "lead-worker",
  "lead-aggregator",
...
async function buildHandler(name) {
  ...
  await build({
    entryPoints: [path.join(projectRoot, "src", "aws-pipeline", "handlers", `${name}.js`)],
...
export async function buildLambdaPackages() {
  await assertRequiredInputs();
  await rm(buildRoot, { recursive: true, force: true });
  await rm(outputRoot, { recursive: true, force: true });
  await mkdir(buildRoot, { recursive: true });
  await mkdir(outputRoot, { recursive: true });
  for (const handlerName of LAMBDA_HANDLERS) await buildHandler(handlerName);
}

if (import.meta.url === pathToFileURL(process.argv[1]).href) await buildLambdaPackages();
